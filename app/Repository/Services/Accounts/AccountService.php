        ...
        try {
            ...
            $accountHistoryList = DB::table('account_history')
                ...
                ->paginate($perPage, ['account_history.*'], 'page', $page);
                 ...
                 $accountHistorySummary = DB::table('account_history')
                ->where('tran_date', '>=', $startDate)
                ->where('tran_date', '<', $endDate)
            ...
            $response = ['accountHistoryList' => $accountHistoryList, 'accountHistorySummary' => $accountHistorySummary];
         
            if($accountHistoryList->total() > 0){
                return ["status" => ApiResponseStatus::SUCCESS, "data" => $response, "message" => config("message.app_data_retrieved")];
            }
